    ...
    message.author.send(embed).catch(err => message.channel.send(erros)).then(async msg => {
        ...
        staffL.on('collect', r => {
            const embeddiversao = new Discord.RichEmbed()
                ...
                !addemoji \`<nome do emoji>\` \`<link>\`    -Adiciona um emoji
                !afk \`<on/off>\`                           -Adiciona o status em si mesmo de afk.
                !aprovar \`<usuário>\` \`<tag cidadão>\`    -Coloca o cargo de whitelist de uma pessoa.
                !aviso \`<descrição>\`                      -Faz um anuncio no canal de Anuncios
                !ban \`<usuário>\` \`<razão>\`              -Bane um usuário.
                !chat \`<on/off>\`                          -Abre/Fecha o chat
                !embed \`<mensagem>\`                       -Manda uma mensagem em embed
                !enquete \`<mensagem>\`                     -Cria uma votação!
                ...
                !quiz \`<premio>\`                          -Cria um quiz!
                !rolemention \`<role>\`                     -Transforma um cargo em mencionavel por certo tempo
