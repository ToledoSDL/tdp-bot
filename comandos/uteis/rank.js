    ...
    Database.Users.find().sort([
        ...
        if (res.length === 0) { //se o resultado for igual a 0
            ...
            for (i = 0; i < res.length; i++) {
                ...
                if (member) {
                    embed.addField(`**Level**: ${res[i].level} - **XP**: ${res[i].xp}`, `**${i + 1}**. ${member}`);
                } else {
                    embed.addField(`**Level**: ${res[i].level} - **XP**: ${res[i].xp}`, `**${i + 1}**. ${member}`);
                }
            }
            ...
            for (i = 0; i < 5; i++) {
                let member = client.users.get(res[i]._id)
                embed.addField(`**Level**: ${res[i].level} - **XP**: ${res[i].xp}`, `**${i + 1}**. ${member}`); //adicionamos na embed o nome e os coins do usuario
            }
        }
