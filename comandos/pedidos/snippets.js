...
exports.run = async (client, message, args) => {
    ...
    const embed = new Discord.RichEmbed()
        .setColor(c.cor)
        .setAuthor("ToledoSDL© - Snippet de código", client.user.avatarURL)
...
\`💡\` Linguagem: ${linguagem}
\`📪\` Código: \`\`\`${linguagem}\n${codigo}\`\`\``)
                                    .setFooter("Quer enviar um snippet? Use `!codigo` na sala `#🤖 comandos` © ToledoSDL©", message.author.avatarURL)
                                client.channels.get("706550860804522114").send(pedido)
