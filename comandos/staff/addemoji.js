...
exports.run = async (client, message, args) => {
    ...
    if (message.member.hasPermission('MANAGE_EMOJIS')) {
        ...
        if (emoteSize < 1) {
            const errourl = new RichEmbed()
                .setTitle('Insira uma URL ao seu emoji')
                .setColor("RANDOM")
                .setDescription(`Você não inseriu uma URL apos o nome do emoji ex.: !addemoji Nomezinho https://urlfoda.png`)
            message.channel.send(errourl)
        }
        if (emoteSize < 256000) {
            const embed = new RichEmbed()
